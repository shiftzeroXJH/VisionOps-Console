...
def main() -> int:
    if len(sys.argv) != 2:
        print("usage: python -m openclaw_yolo.core.validation_worker <request_json>", file=sys.stderr)
        return 2
...
def _parse_dataset_yaml(dataset_yaml: Path) -> dict[str, Any]:
    from openclaw_yolo.core.dataset import _parse_dataset_yaml as parse_dataset_yaml

    return parse_dataset_yaml(dataset_yaml)


def _resolve_dataset_base_path(dataset_yaml: Path, raw_path: Any) -> Path:
    from openclaw_yolo.core.dataset import _resolve_dataset_base_path as resolve_dataset_base_path
...
def _resolve_split_path(base_path: Path, yaml_dir: Path, split_value: str) -> Path:
    from openclaw_yolo.core.dataset import _resolve_split_path as resolve_split_path

    return resolve_split_path(base_path, yaml_dir, split_value)


def _resolve_label_dir(image_dir: Path, split_value: str) -> Path:
    from openclaw_yolo.core.dataset import _resolve_label_dir as resolve_label_dir

    return resolve_label_dir(image_dir, split_value)


def _normalize_names(raw_names: Any) -> dict[int, str]:
    from openclaw_yolo.core.dataset import _normalize_names as normalize_names
